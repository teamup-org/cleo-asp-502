...
class DegreePlannerController < ApplicationController
  ...
  def search_params
    params.permit(:ccode, :cnumber, :credit_hours, :sort_by, :direction, :page)
  end
  ...
  def earliest_semester_for_course(student, course)
    ...
    prereq_groups.each do |equi_id, prereq_courses|
      prereq_courses.each do |prereq|
        prereq_course_semester = courses_by_semester.find do |sem, courses|
          courses.map(&:course).include?(prereq)
        end&.first || 0

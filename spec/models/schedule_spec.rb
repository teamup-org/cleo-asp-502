...
RSpec.describe Schedule, type: :model do
    ...
    student = Student.find_or_create_by!(
        ...
        it 'is valid with valid attributes' do
            schedule = Schedule.create(student:student, semester: 1)
            expect(schedule).to be_valid
        end
    end
    context 'When creating a invalid schedule' do
        it 'is invalid with invalid attributes' do
            schedule2 = Schedule.create(student:student, semester: 'test')
            expect(schedule2).to be_invalid
        end
    end
    context 'When creating a invalid schedule' do
        it 'is invalid with duplicate semester,student_google_id' do
            schedule2 = Schedule.create(student:student, semester: 2)
            schedule3 = Schedule.create(student:student, semester: 2)
